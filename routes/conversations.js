...
router.get('/messages', function(req, res){
  ...
  if(req.query.to){
    ...
    Message.find({
      $and: [ { messageIndex: { $lte: req.query.to}}, {messageIndex: { $gte: req.query.from}}, {conversationId: req.query.conversationId}],
    ...
    Message.find({
      $and: [ {conversationId: req.query.conversationId}, {messageIndex: { $gte: req.query.from } }]
    }, function(err, messages){
      if(err) throw err;
